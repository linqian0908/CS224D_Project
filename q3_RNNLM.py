...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      inputs=[tf.squeeze(t,[1]) for t in tf.split(1,self.config.num_steps,embed)]
      ### END YOUR CODE
      return inputs
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    loss = sequence_loss([output],[tf.reshape(self.labels_placeholder,[-1])],\
    ...
    with tf.variable_scope('RNN') as scope:
        ...
        for e in inputs:
            if len(rnn_outputs)==0:
                ...
                h=rnn_outputs[-1]
            dropped_h=tf.nn.dropout(h,self.dropout_placeholder)
            dropped_e=tf.nn.dropout(e,self.dropout_placeholder)
            rnn_outputs.append(tf.sigmoid(tf.matmul(dropped_h,H)+tf.matmul(dropped_e,I)+b1))
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    y_pred,state = session.run([model.predictions[-1],model.final_state],\
